...
class JsnRegisterdateFieldHelper
{
	...
	{
		$data->registerdate=$data->registerDate;
	}
	...
	public static function getSearchQuery($field, &$query)
	{
		...
		if(JRequest::getVar($field->alias.'_from','')!='') $query->where('a.'.$db->quoteName('registerDate').' > '.$db->quote($date_from->toSql()));
		if(JRequest::getVar($field->alias.'_to','')!='') $query->where('a.'.$db->quoteName('registerDate').' < '.$db->quote($date_to->toSql()));
	}
