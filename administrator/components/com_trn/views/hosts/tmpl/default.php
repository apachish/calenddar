...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_trn&task=hosts.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'hostList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
...
<?php endif;?>
		...
		<table class="table table-striped" id="hostList">
			...
			<?php foreach ($this->items as $i => $item) :
                ...
                <?php if (isset($this->items[0]->state)): ?>
					<td class="center">
						<?php echo JHtml::_('jgrid.published', $item->state, $i, 'hosts.', $canChange, 'cb'); ?>
					</td>
                <?php endif; ?>
				...
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_trn&task=host.edit&id='.(int) $item->id); ?>">
