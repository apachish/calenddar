...
class JsnModelField extends JModelAdmin
{
	...
	protected function canEditState($record,$nameField = null)
	{
		...
		if(($record->core && $nameField!='profile' && $nameField!='search') && !($record->alias=='avatar' && ($nameField=='edit' || $nameField=='required' || $nameField=='register')))
		{
			JError::raiseWarning(403, JText::_('JLIB_APPLICATION_ERROR_EDITSTATE_NOT_PERMITTED').' - '.JText::_('COM_JSN_VIEWCONFIG'));
			return false;
		}
	...
	*/
	...
	public function save($data)
	{
		...
		if(!$isNew)
		{
			...
			unset($data['alias']);
			unset($data['core']);
			if($table->core && $table->alias != 'avatar'){
				unset($data['required']);
				//unset($data['profile']);
				unset($data['register']);
				unset($data['edit']);
		...
		$reserved=array('id','name','email1','email2','password1','password2');
		if($isNew && in_array( $data['alias'] , $reserved ))
		{
			$data['alias']='jsn-'.$data['alias'];
		}
	...
	*/
	...
	public function toggle(&$pks, $value = 1, $nameField)
	{
		...
		foreach ($pks as $i => $pk)
		{
			if ($table->load($pk))
			{
				/*if($table->type=='tos' && in_array($nameField,array('profile','register','required'))){
					unset($pks[$i]);
