...
<?php if (!empty( $this->sidebar)): ?>
	<div id="j-sidebar-container" class="span2">
		...
		?>
			<iframe style="border:none;padding:0px;margin:px" class="hidden-phone" src="http://www.easy-profile.com/version.php?t=free&amp;v=<?php echo $versionNumber; ?>&amp;n=<?php echo $version; ?>" ></iframe>
		<?php endif; ?>
	</div>
			...
			foreach ($this->items as $i => $item) :
				...
				?>
					<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->parent_id;?>" item-id="<?php echo $item->id?>" parents="<?php echo $parentsStr?>" level="<?php echo $item->level?>" <?php if($item->level==1) echo('style="background:#f9f9f9"'); ?> >
						...
						<td>
							<?php if($item->level==2) echo JHtml::_('field.required', $item->required, $i, ($item->core && $item->alias!='avatar' ? false : $canChange) );?>
						</td>
						<td>
